import { c as create_ssr_component, h as escape, f as add_attribute, v as validate_component } from "../../../../../../../chunks/ssr.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `  ${$$result.head += `<!-- HEAD_svelte-18pkd8t_START -->${$$result.title = `<title>Blog category ${escape(category)} - page ${escape(page)}</title>`, ""}<meta data-key="description"${add_attribute("name", siteDescription, 0)}><!-- HEAD_svelte-18pkd8t_END -->`, ""}  ${posts && posts.length ? `<h1 class="text-2xl md:text-4xl mx-auto text-center py-4 ">Category: ${escape(category)} <br> <div class="text-lg my-3">Posts ${escape(lowerBound)}–${escape(upperBound)} of ${escape(totalPosts)}</div></h1> ${validate_component(Pagination, "Pagination").$$render(
    $$result,
    {
      currentPage: page,
      totalPosts,
